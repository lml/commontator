class CommentThread::Thread < ActiveRecord::Base
  ...
  def clear
    new_thread = CommentThread::Thread.new
    new_thread.commentable = commentable
    CommentThread::Thread.transaction do
      new_thread.save!
  ...
  def subscription_for(subscriber)
    CommentThread::Subscription.find_by_subscriber_id_and_subscriber_type_and_comment_thread_id(\
      subscriber.id, subscriber.class_name, id)
  end

  def subscribe!(subscriber)
    return false if subscription_for(subscriber)
    subscription = CommentThread::Subscription.create(
